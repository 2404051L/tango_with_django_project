...
def index(request):
    ...
    #Retrieve te top 5 only -- or all if less than 5.
    #Place the list un our context_dict dictionary (with our boldmessage!)
    ...
    #Return the reponse back to the user, updating any cookies that need changed.
    return response
...
def add_category(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            #Now that thecategory is saved, we could confirm this.
...
def register(request):
    ...
    if request.method == 'POST':
        #Attempted to gra information from the raw form information.
    ...
    #render the remplate depending on the context.
    return render(request, 'rango/register.html', context= {'user_form': user_form, 'profile_form': profile_form, 'registered': registered})
            
def user_login(request):
    #If the request is a HTTP POST, tryt to pull out the relevant information.
    if request.method == 'POST':
        ...
        if user:
            #Is the account active? It could have een disabled.
            if user.is_active:
